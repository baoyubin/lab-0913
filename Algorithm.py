...
if __name__ == '__main__':
    ...
    if(run_mode == 1):
        env = Env(map_x, map_y, taxi_map.load_map, bus_map.bus_map, my_plot, delay_Weight=0.8, idle_w=0.5, data_num=3)
    else:
    ...
    agent = Agent(n_state, n_action, GAMA=0.9, learning_race=1e-3)
    if(run_mode == 1):
    ...
    if algorithm_mode == "DDQN":
        torch.save(agent.online_net.state_dict(),
               'Algorithm/ddqn/net_params_' + str(env.delay_Weight) + '_' + str(n_episode)
               + '_' + str(env.data_num) + '_' + lllll + '.pkl')  # 只保存网络中的参数

        np.savetxt('Algorithm/ddqn/' + lllll + '_' + str(env.delay_Weight) + '_' + str(n_episode) + '_' + str(
        ...
        torch.save(agent.online_net.state_dict(), 'Algorithm/action/net_params_' + str(env.delay_Weight) + '_' + str(n_episode)
               + '_' + str(env.data_num) + '_' + lllll + '.pkl')  # 只保存网络中的参数

        np.savetxt('Algorithm/action/' + lllll + '_' + str(env.delay_Weight) + '_' + str(n_episode) + '_' + str(env.data_num) + '_' + str(env.B_ecd/env.B_bus)
               + '_' + str(env.ECD_CPU_frequency / env.BUS_CPU_frequency) + '_' + str(env.bus_bound) + '.txt', [env.my_plot.reward_list[-107:], env.my_plot.rw_all_1[-107:], env.my_plot.rw_all_0[-107:], energy, delay], fmt = '%s')